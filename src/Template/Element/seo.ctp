 ...
 */
    ...
    if (isset($_request_params['pass'])&&(!empty($_request_params['pass']))){
        $seo_key_detail = $seo_key.$separator.strtolower(implode($separator,$_request_params['pass']));
    
        if (\Cake\Core\Configure::check('Seo.'.$seo_key_detail)){
            $this->start('title');
            echo \Cake\Core\Configure::read('Seo.'.$seo_key_detail.'.title'),' - ',$_sys_info['name'];
            //echo __d('seo', \Cake\Core\Configure::read('Seo.'.$seo_key.'.title'));//多语言
            $this->end();
            ...
            echo $this->Html->meta(
                'keywords',
                \Cake\Core\Configure::read('Seo.'.$seo_key_detail.'.keywords')
                //__d('seo', \Cake\Core\Configure::read('Seo.'.$seo_key.'.keywords'))//多语言
                );
            echo $this->Html->meta(
                'description',
                \Cake\Core\Configure::read('Seo.'.$seo_key_detail.'.description')
                //__d('seo', \Cake\Core\Configure::read('Seo.'.$seo_key.'.description'))//多语言
                );
            $this->end();
        }
